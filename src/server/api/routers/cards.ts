...
export const cardsRouter = createTRPCRouter({
  getAll: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const [providers, banks, picachus] =
        await Promise.all([
      ...
      const processedCards = cards.map(card => {
        ...
        const totalPoured = card.cardPouring.reduce((sum, pouring) => 
          sum + pouring.pouringAmount, 0);
          
        // Расчет конечного баланса (последний баланс по дате)
        const lastBalance = card.balances.length > 0 
          ? card.balances.sort((a, b) => 
              new Date(b.date).getTime() - new Date(a.date).getTime())[0].endBalance 
          : 0;
          
        // Расчет начального баланса (самый ранний баланс по дате)
        const initialBalance = card.balances.length > 0 
          ? card.balances.sort((a, b) => 
              new Date(a.date).getTime() - new Date(b.date).getTime())[0].startBalance 
          : 0;
  ...
  update: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const parsedData = {
        ...updateData,
        cardPrice: typeof updateData.cardPrice === 'string' 
  ...
  getAuditLogs: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      const { entityType, entityId, userId, page, pageSize, startDate, endDate } = input;
