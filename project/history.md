...
## 04.04.2025 15:39
- Дополнены функции таблицы карт и исправлены ошибки:
  - Исправлены фильтры по полям "letterCode" и "inWork"
  - Добавлено поле "Актер" (actor) в формы создания и редактирования карт
  - Добавлена кнопка для просмотра всей истории изменений в системе
  - Реализован новый API метод для получения всех записей аудита
  - Обновлен компонент AuditLogDialog для поддержки просмотра всех записей
  - Добавлено отображение поля "Поставщик" в детализации карты

## 04.04.2025 15:33
- Добавлены новые функции для таблицы карт:
  - Добавлен letterCode в качестве второго столбца таблицы
  - Реализована возможность фильтрации по letterCode
  - Добавлено поле "В работе" (inWork) для карт с возможностью редактирования и фильтрации
  - Обновлена детальная информация о карте с использованием компонента StatusBadge
  - Добавлена поддержка сортировки по полю "В работе"
  - Внесены соответствующие изменения в API и модель данных

## 04.04.2025 15:25
- Добавлено сохранение активного метода оплаты в базе данных:
  - Добавлено поле `activePaymentMethod` в модель Card в схеме Prisma
  - Создан новый API метод updatePaymentMethod в роутере карт
  - Реализовано сохранение выбранного метода при клике на номер карты или телефон
  - Добавлено ведение аудит-лога изменений активного метода оплаты
  - Исправлена ошибка синтаксиса TypeScript в определении типа активных методов оплаты

## 04.04.2025 15:23
- Усовершенствована таблица карт - добавлены отдельные столбцы для C2C и СБП:
  - Теперь оба метода оплаты (C2C и СБП) отображаются одновременно в отдельных столбцах
  - Добавлена возможность переключения активного метода оплаты кликом по номеру
  - Активный метод подсвечивается зеленым цветом и отмечается галочкой
  - Неактивный метод подсвечивается красным цветом
  - Добавлена инструкция по использованию этой функции

## 04.04.2025 15:18
- Обновлена страница карт согласно новым требованиям:
  - Заменены компоненты HeroUI на shadcn/ui
  - Изменен набор столбцов в таблице карт: ID, банк, номер карты/телефон, балансы, сумма пролива, сумма выплат
  - Добавлен новый компонент CardDetailsDialog для просмотра подробной информации о карте
  - Обновлены импорты для использования синтаксиса алиаса путей ~/
  - Аудит изменений для балансов карт уже реализован в существующем коде

## 04.04.2025 10:22
- Исправлена структура связи между WorkSession и IdexCabinet:
  - Реализована связь many-to-many через промежуточную таблицу WorkSessionIdexCabinet
  - Теперь кабинеты могут находиться одновременно в нескольких сессиях
  - Исправлен расчет продолжительности сессии для корректного отображения временных интервалов
  - Добавлена возможность редактирования кабинетов (добавления/удаления) в завершенных сессиях
  - Обновлены API-роутеры для поддержки новой структуры данных
  - Исправлена ошибка в IdexCabinetRouter, использовавшем удаленное поле workSessionId
...
=======
## 01.04.2025 22:55
- Исправлена критическая ошибка при добавлении баланса:
  - Добавлено поле `comment` в модель CardBalance в схеме Prisma
  - Необходимо запустить миграцию для применения изменений в базе данных

## 01.04.2025 22:45
- Исправлена ошибка с кнопкой добавления баланса:
  - Обновлен API-метод cardBalances.create для правильной обработки полей формы
  - Исправлены типы данных для удаления и обновления балансов (id теперь числовой)
  - Добавлена поддержка комментариев в балансах
  - Улучшена обработка аудита для балансов с корректной связью между сущностями

## 01.04.2025 22:30
- Исправлена ошибка с кнопкой создания карты:
  - Устранено несоответствие между схемой валидации формы и API-роутером
  - Исправлена обработка поля appPin (теперь передается как число, а не undefined)
  - Добавлена корректная обработка начальных проливов и балансов при создании карты
  - Улучшена валидация данных в методе create в API-роутере cards.ts
  - Добавлена поддержка для одного из двух значений полей pouringAmount/initialAmount

## 01.04.2025 21:25
- Обновлены компоненты для работы с картами:
  - Добавлено поле letterCode (буквенный код) в оба диалоговых окна (создания и редактирования карт)
  - Дополнены недостающие поля в окне редактирования карты для соответствия функционалу окна создания
  - Обновлен интерфейс Card для поддержки нового поля
  - Поле буквенного кода размещено первым в интерфейсе обоих форм
  - Улучшена компоновка полей для более удобного использования

## 01.04.2025 21:15
- Расширен функционал карт с добавлением новых полей:
  - Добавлено поле letterCode (буквенный код) в модель Card в схеме Prisma
  - Обновлен интерфейс CardType для поддержки нового поля
  - Реализовано редактирование поля letterCode в форме карточки
  - Обновлены API-методы для поддержки нового поля (create и update)
  - Обновлена структура компонентов для отображения нового поля

## 01.04.2025 19:40
- Добавлены дополнительные функции для работы с пользователями:
  - Возможность добавлять примечания к аккаунтам пользователей
  - Улучшен интерфейс управления статусом аккаунта
  - Оптимизирована загрузка списка пользователей
  - Добавлены новые фильтры поиска
>>>>>>> 592d0cee5c26d3764763a97c7ad03b2192e63492
